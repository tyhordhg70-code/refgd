...
type RevealVariant =
  ...
  | "tilt-3d"      // 3D Y-axis flip-in (toned down)
  | "wipe-diag"    // ACTUAL diagonal clip-path wipe (top-left → bottom-right)
  | "blur-up"      // depth-of-field rise: blurred + low → focused & in place
  | "skew-rise";   // mass-and-momentum rise with un-skew
...
export default function GlassCard({
  ...
  // Pool order chosen so adjacent index'd cards animate distinctly:
  //   index 0 = blur-up (calm focal arrival)
  //   index 1 = slide-left
  //   index 2 = curtain (taller, dramatic)
  //   index 3 = wipe-diag
  //   index 4 = slide-right
  //   index 5 = skew-rise
  //   index 6 = tilt-3d
  //   index 7 = iris
  // The mix interleaves clip-path wipes, 3D rotations, and physical
  // slides so a feature grid never feels mechanical.
  ...
  const initials: Record<RevealVariant, any> = {
    curtain:       { opacity: 0, y: 110, rotateX: 6, clipPath: "inset(100% 0% 0% 0%)" },
    "slide-left":  { opacity: 0, x: -110, clipPath: "inset(0% 0% 0% 100%)" },
    "slide-right": { opacity: 0, x:  110, clipPath: "inset(0% 100% 0% 0%)" },
    // iris: full clip but with bigger final radius so corners reveal
    iris:          { opacity: 0, scale: 0.9,  clipPath: "circle(0% at 50% 50%)" },
    // Toned tilt-3d: less extreme rotateY so it doesn't read as "broken"
    "tilt-3d":     { opacity: 0, rotateY: -26, x: -30 },
    // True diagonal wipe (top-left corner expands to bottom-right)
    "wipe-diag":   { opacity: 0, clipPath: "polygon(0 0, 0 0, 0 0, 0 0)" },
    // Depth-of-field rise — most subtle, used as default at index 0
    "blur-up":     { opacity: 0, y: 60, filter: "blur(18px)" },
    // Mass-and-momentum rise with un-skew
    "skew-rise":   { opacity: 0, y: 80, skewY: 4 },
  };
  const targets: Record<RevealVariant, any> = {
    ...
    // 150% guarantees the iris fully reveals every corner of the card
    // (75% used to leave the four corners permanently clipped — that
    // was the "weird" missing-corners look the user reported).
    iris:          { opacity: 1, scale: 1, clipPath: "circle(150% at 50% 50%)" },
    "tilt-3d":     { opacity: 1, rotateY: 0, x: 0 },
    // Diagonal wipe ends as a full-cover diamond → rectangle.
    "wipe-diag":   { opacity: 1, clipPath: "polygon(0 0, 140% 0, 100% 140%, 0 100%)" },
    "blur-up":     { opacity: 1, y: 0, filter: "blur(0px)" },
    "skew-rise":   { opacity: 1, y: 0, skewY: 0 },
  };
  const durations: Record<RevealVariant, number> = {
    curtain: 1.5, "slide-left": 1.15, "slide-right": 1.15,
    iris: 1.3, "tilt-3d": 1.25, "wipe-diag": 1.1,
    "blur-up": 1.1, "skew-rise": 1.15,
  };
