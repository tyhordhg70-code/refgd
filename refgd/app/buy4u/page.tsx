...
const LOGO_SRC: Record<string, string> = {
  ...
  "amtrak.com": "https://upload.wikimedia.org/wikipedia/commons/thumb/e/eb/Amtrak_logo.svg/330px-Amtrak_logo.svg.png",
  ...
  "booking.com": "https://upload.wikimedia.org/wikipedia/commons/thumb/b/be/Booking.com_logo.svg/330px-Booking.com_logo.svg.png",
  ...
  "avis.com": "https://upload.wikimedia.org/wikipedia/commons/thumb/f/fd/Avis_logo.svg/330px-Avis_logo.svg.png",
  "budget.com": "https://upload.wikimedia.org/wikipedia/commons/thumb/8/8f/Budget_logo.svg/330px-Budget_logo.svg.png",
  "dominos.com": "https://upload.wikimedia.org/wikipedia/commons/thumb/7/74/Dominos_pizza_logo.svg/330px-Dominos_pizza_logo.svg.png",
  "popeyes.com": "https://upload.wikimedia.org/wikipedia/commons/thumb/7/73/Popeyes_logo.svg/330px-Popeyes_logo.svg.png",
...
const SECTIONS: Section[] = [
  ...
  { id:"attractions",label:"Attractions & Tours",icon:"🎡",hasRefund:true,buy4u:{
    ...
    cards:[
      ...
      C("a24","Excursions","https://images.unsplash.com/photo-1530841377377-3ff06c0ca713?auto=format&fit=crop&w=800&q=60"),
  ...
  return (<div>
    {service ? (
      ...
      <>
        <div
          role="button"
          tabIndex={isAdmin && editMode ? -1 : 0}
          aria-disabled={isAdmin && editMode}
          aria-label="Open the Hotels insider instant refunds guide"
          aria-haspopup="dialog"
          aria-expanded={hotelsInfoOpen}
          onClick={() => { if (isAdmin && editMode) return; setHotelsInfoOpen(true); }}
          onKeyDown={(e) => { if (isAdmin && editMode) return; if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setHotelsInfoOpen(true); } }}
          className={`group flex w-full items-center justify-between gap-4 rounded-xl border border-amber-300/30 bg-amber-400/[0.06] px-4 py-3.5 text-left transition hover:border-amber-300/50 hover:bg-amber-400/[0.1]${isAdmin && editMode ? "" : " cursor-pointer"}`}
        >
          <span className="min-w-0">
            <EditableText id={`buy4u.${section.id}.hotelsinfo.title`} defaultValue="HOTELS.com / EXPEDIA / AGODA / TRIP.com — INSIDER INSTANT REFUNDS" as="span" className="block text-base font-bold leading-snug text-amber-100" />
            <EditableText id={`buy4u.${section.id}.hotelsinfo.subtitle`} defaultValue="Tap to read the full insider refund guide — enjoy your vacation at a fraction of the price." as="span" className="mt-0.5 block text-sm font-medium text-white/70" />
          </span>
...
const GC_CATEGORIES = ["Food And Restaurants","Gas Stations","Retail","Groceries","Entertainment"];
const GC_AVAILABILITY = ["In Stock","Best Selling","Instant Delivery","PIN","PDF","Pass2U","Online","In-Store"];
const GC_STATES = ["AL","AK","AS","AZ","AR","CA","CO","CT","DE","DC","FM","FL","GA","GU","HI","ID","IL","IN","IA","KS","KY","LA","ME","MH","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","MP","OH","OK","OR","PW","PA","PR","RI","SC","SD","TN","TX","UT","VT","VI","VA","WA","WV","WI","WY"];
...
const GC_CARDS: GiftCard[] = [
  {key:"sgc189",name:"$15s Applebee's",brand:"$15s Applebee's",category:"Food And Restaurants",states:["AL","AK","AZ","AR","CA","CO","CT","DE","DC","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"],availability:["In Stock","PDF"],image:"/gc-img/7b8d7df6-dbf9-4cc6-9bf4-baf4b423e3c6.webp",price:"55% off"},
  {key:"sgc181",name:"Amici's East Coast Pizzeria",brand:"Amici's East Coast Pizzeria",category:"Food And Restaurants",states:["CA"],availability:["In Stock"],price:"49% off"},
  {key:"sgc144",name:"Applebee's [SALE]",brand:"Applebee's [SALE]",category:"Food And Restaurants",states:["AL","AK","AZ","AR","CA","CO","CT","DE","DC","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"],availability:["In Stock","Best Selling","PDF"],image:"/gc-img/a6639074-d7ad-45cc-9287-5f1668061a75.webp",price:"55% off"},
  {key:"sgc185",name:"Arby's",brand:"Arby's",category:"Food And Restaurants",states:["AL","AL","AZ","AR","CA","CO","CT","DE","FL","GA","HI","IL","IN","IA","KA","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","SC","SD","TN","TX","UT","VA","WA","WV","WI","WY"],availability:["In Stock"],price:"45% off"},
  {key:"sgc013",name:"Bad Daddy's Burger Bar",brand:"Bad Daddy's Burger Bar",category:"Food And Restaurants",states:["AL","CO","OK","SC"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/7bfc718d-8203-4ac7-8575-9f7bc7a5e4b9.webp",price:"50% off"},
  {key:"sgc014",name:"Bar Louie",brand:"Bar Louie",category:"Food And Restaurants",states:["AZ","AR","CA","CO","CT","DC","FL","IL","IN","IA","KY","MD","MA","MI","MN","MO","NJ","NY","OH","PA","RI","SC","TN","TX","VA","WI"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/0a7385c3-2968-4025-9532-03b47219d87d.webp",price:"50% off"},
  {key:"sgc015",name:"Beef O Brady's",brand:"Beef O Brady's",category:"Food And Restaurants",states:["AL","AR","FL","GA","IL","KY","LA","MD","MI","MN","MS","MO","MT","NC","OH","SC","TX","VA","WI"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/daccfbde-00ce-4e47-a1a2-75fcc7847c4a.webp",price:"50% off"},
  {key:"sgc149",name:"Ben & Jerry's [Check Balance]",brand:"Ben & Jerry's [Check Balance]",category:"Food And Restaurants",states:["CA","CO","CT","DC","FL","GA","HI","IL","ME","MD","MA","MI","MN","MS","MO","NV","NH","NJ","NY","NC","OH","OR","PA","RI","SC","TN","TX","VT","VA","WA"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/78d1f5e9-8dd8-4f21-a139-05bc25282b13.webp",price:"60% off"},
  {key:"sgc018",name:"Black Angus Steakhouse",brand:"Black Angus Steakhouse",category:"Food And Restaurants",states:["AK","AZ","CA","HI","NM","WA"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/da1bdb33-e8d5-4a86-862b-8bcdbd717fd7.webp",price:"60% off"},
  {key:"sgc019",name:"Bloomin Brands (aussie Grill, Outback Steakhouse, Carrabba's, Fleming's Steakhouse, Bonefish Grill)",brand:"Bloomin Brands (aussie Grill, Outback Steakhouse, Carrabba's, Fleming's Steakhouse, Bonefish Grill)",category:"Food And Restaurants",states:["AL","AK","AZ","AR","CA","CO","CT","DE","DC","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","PR","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"],availability:["Out of Stock","PIN","PDF","Online","In-Store"],image:"/gc-img/06115478-3e71-4975-8333-bf1e1a6b43f8.webp"},
  {key:"sgc020",name:"Blue Martini Lounge",brand:"Blue Martini Lounge",category:"Food And Restaurants",states:["AZ","FL","GA","LA","NV"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/7ef6bf21-5803-4b93-b9ac-50f6dcbcd4e5.webp",price:"60% off"},
  {key:"sgc021",name:"Bob Evans",brand:"Bob Evans",category:"Food And Restaurants",states:["AL","AR","DE","FL","IN","IA","KS","KY","MD","MA","MI","MS","MO","NJ","NY","NC","OH","PA","SC","TN","TX","VA","WV"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/aa9df2a3-9f8e-404f-82d9-394b51d1f5cb.webp",price:"55% off"},
  {key:"sgc023",name:"Buca Di Beppo (bravo/brio, Bertucci's)",brand:"Buca Di Beppo (bravo/brio, Bertucci's)",category:"Food And Restaurants",states:["AZ","CA","CO","DC","FL","GA","IL","IN","KY","MD","MI","MN","MO","NV","NJ","NM","NY","NC","OH","PA","TN","TX","UT"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/f2b92853-8503-402f-8efa-91306420f7ef.webp",price:"55% off"},
  {key:"sgc024",name:"Buffalo Wild Wings",brand:"Buffalo Wild Wings",category:"Food And Restaurants",states:["AL","AK","AZ","AR","CA","CO","CT","DE","DC","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"],availability:["In Stock","Best Selling","PDF","In-Store"],image:"/gc-img/7189b5d2-3a67-4820-a9a6-e84a602af1d5.webp",price:"35% off"},
  {key:"sgc026",name:"Burgerfi",brand:"Burgerfi",category:"Food And Restaurants",states:["AL","AK","AZ","CA","CO","FL","GA","IL","IN","KY","MD","NV","NJ","NC","OH","TN","TX","VT","VA"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/9211a695-e913-4b4d-9484-fffc0be7db3b.webp",price:"60% off"},
  {key:"sgc027",name:"Cafe Rio Mexican Grill",brand:"Cafe Rio Mexican Grill",category:"Food And Restaurants",states:["AZ","CA","CO","DC","ID","MD","MT","NV","VA","WA","WY"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/226a039b-7d93-4970-8a2a-cc251063d786.webp",price:"50% off"},
  {key:"sgc182",name:"Cafe Zupas",brand:"Cafe Zupas",category:"Food And Restaurants",states:["AZ","ID","IL","MN","NV","OH","UT","WI"],availability:["In Stock"],price:"49% off"},
  {key:"sgc028",name:"California Pizza Kitchen [CPK]",brand:"California Pizza Kitchen [CPK]",category:"Food And Restaurants",states:["AL","AZ","CA","CO","CT","DC","FL","GA","HI","IL","IN","IA","KS","KY","LA","MD","MA","MI","MN","MO","NE","NV","NJ","NM","NY","NC","OH","OR","PA","TN","TX","UT","VA","WA","WI"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/e0be7557-6aac-49b1-94a7-d103d1d6dc19.webp",price:"39% off"},
  {key:"sgc029",name:"Captain D's Seafood",brand:"Captain D's Seafood",category:"Food And Restaurants",states:["AL","AR","CO","FL","GA","IL","IN","KS","KY","LA","MI","MS","MO","NM","NC","OH","OK","SC","TN","TX","VA","WV"],availability:["In Stock","PDF","Online","In-Store"],image:"/gc-img/914acf9c-30f2-4aea-bbde-3cc9b8d0993e.webp",price:"50% off"},
  {key:"sgc030",name:"Caribou Coffee",brand:"Caribou Coffee",category:"Food And Restaurants",states:["CO","FL","GA","IL","IN","IA","KS","MI","MN","MO","NE","NC","ND","OH","OK","SC","SD","TX","WI","WY"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/6044fc53-c48b-475a-8645-c1faba758a9e.webp",price:"55% off"},
  {key:"sgc172",name:"Cava",brand:"Cava",category:"Food And Restaurants",states:["AL","AZ","AR","CA","CO","CT","DE","DC","FL","GA","IL","IN","KS","LA","MD","MA","MI","MO","NH","NJ","NY","NC","OH","PA","RI","SC","TN","TX","VA"],availability:["In Stock","Best Selling","PIN","Pass2U","In-Store"],price:"41% off"},
  {key:"sgc031",name:"Cheba Hut",brand:"Cheba Hut",category:"Food And Restaurants",states:["AZ","CA","CO","FL","GA","IL","LA","NV","NM","OH","OR","TX","WI"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/4aa49572-3a34-454f-9a1c-2cebe969a888.webp",price:"55% off"},
  {key:"sgc032",name:"Chicken Salad Chick",brand:"Chicken Salad Chick",category:"Food And Restaurants",states:["AL","AR","FL","GA","IL","KY","LA","MS","MO","NC","OH","OK","SC","TX","VA"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/5fefc6dc-302c-40d4-a015-ee07df2fc8b3.webp",price:"60% off"},
  {key:"sgc033",name:"Chuy's Tex-mex",brand:"Chuy's Tex-mex",category:"Food And Restaurants",states:["AL","AR","CO","FL","GA","IL","IN","KS","KY","MO","NC","OH","OK","SC","TN","TX","VA"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/d8e633d3-a043-4800-aa70-dcfbe2627aef.webp",price:"45% off"},
  {key:"sgc186",name:"Co",brand:"Co",category:"Food And Restaurants",states:["GA","SC","NC","SC"],availability:["In Stock"],price:"53% off"},
  {key:"sgc035",name:"Condado Tacos",brand:"Condado Tacos",category:"Food And Restaurants",states:["AL","IN","KY","MI","MO","NC","OH","PA","SC","TN"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/241f5b04-f2a4-41bf-875c-812030717d45.webp",price:"45% off"},
  {key:"sgc036",name:"Cooper's Hawk Winery & Restaurant",brand:"Cooper's Hawk Winery & Restaurant",category:"Food And Restaurants",states:["AZ","FL","IL","IN","MD","MI","MO","OH","VA","WI"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/46fcbbf7-aa9a-4855-8ea1-76832f5477ea.webp",price:"50% off"},
  {key:"sgc038",name:"Cracker Barrel",brand:"Cracker Barrel",category:"Food And Restaurants",states:["AL","AZ","AR","CA","CO","CT","DE","FL","GA","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","RI","SC","SD","TN","TX","UT","VA","WV","WI"],availability:["In Stock","Best Selling","PDF","In-Store"],image:"/gc-img/8fe46634-a9ac-4174-855d-be52a24a287c.webp",price:"50% off"},
  {key:"sgc040",name:"Denny's",brand:"Denny's",category:"Food And Restaurants",states:["AL","AK","AZ","AR","CA","CO","CT","DC","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/2c4081fc-c335-4dab-abac-7e7b99d23b08.webp",price:"40% off"},
  {key:"sgc041",name:"Dickey's Barbecue Pit",brand:"Dickey's Barbecue Pit",category:"Food And Restaurants",states:["AL","AZ","AR","CA","CO","DE","FL","GA","HI","ID","IL","IN","KS","KY","LA","MI","MN","MS","MO","MT","NE","NV","NJ","NM","NY","NC","ND","OH","OK","OR","PA","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/bb5fdc11-e0c9-404f-9392-c28a7c846ea5.webp",price:"55% off"},
  {key:"sgc042",name:"Din Tai Fung",brand:"Din Tai Fung",category:"Food And Restaurants",states:["CA","NV","NY","OR","WA"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/989afe6f-f4ee-4a3d-b29c-93ee2bee5a40.webp",price:"35% off"},
  {key:"sgc043",name:"Dion's",brand:"Dion's",category:"Food And Restaurants",states:["CO","NM","TX"],availability:["In Stock","PIN","PDF"],image:"/gc-img/52d15d91-8c26-4fcc-b540-f23668373594.webp",price:"45% off"},
  {key:"sgc044",name:"Dog Haus",brand:"Dog Haus",category:"Food And Restaurants",states:["AZ","CA","CO","CT","FL","IL","IN","MD","MA","MO","NV","NY","TX","UT","VA","WV","WY"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/1d5b0a19-30a3-4e45-bcb9-d9fd9368de72.webp",price:"55% off"},
  {key:"sgc045",name:"Duffy's Mvp Grill",brand:"Duffy's Mvp Grill",category:"Food And Restaurants",states:["FL"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/0ad80195-4f75-4524-9377-0a7a88e0f0f8.webp",price:"45% off"},
  {key:"sgc046",name:"Dunkin & Baskin [Semi-Permanent]",brand:"Dunkin & Baskin [Semi-Permanent]",category:"Food And Restaurants",states:["AL","AZ","AR","CA","CO","CT","DE","DC","FL","GA","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","PA","RI","SC","TN","TX","UT","VT","VA","WV","WI"],availability:["In Stock","PIN","PDF","In-Store"],image:"/gc-img/9bf795fd-f8c2-46b5-a569-ef21e45c3b0c.webp",price:"35% off"},
  {key:"sgc047",name:"Dutch Bros Coffee",brand:"Dutch Bros Coffee",category:"Food And Restaurants",states:["AL","AZ","CA","CO","ID","KS","NV","OK","OR","TX","WA"],availability:["In Stock","Best Selling","PDF","In-Store"],image:"/gc-img/d1d649b6-8f13-45b4-8cbf-2447a3b58780.webp",price:"40% off"},
  {key:"sgc179",name:"Eddie Merlot's",brand:"Eddie Merlot's",category:"Food And Restaurants",states:["AZ","CO","IL","IN","MA","MI","OH","PA","VA"],availability:["In Stock","PDF"],price:"45% off"},
  {key:"sgc048",name:"Einstein's Bros Bagels",brand:"Einstein's Bros Bagels",category:"Food And Restaurants",states:["AL","AZ","AR","CA","CO","CT","DE","DC","FL","GA","ID","IL","IN","IA","KS","KY","LA","MD","MA","MI","MN","MS","MO","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","SC","SD","TN","TX","UT","VT","VA","WA","WI","WY"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/28751441-3d1d-4ce1-882d-348410284259.webp",price:"40% off"},
  {key:"sgc173",name:"Famous Dave's",brand:"Famous Dave's",category:"Food And Restaurants",states:["AZ","CA","CO","FL","ID","IL","IN","IA","KS","KY","MD","MI","MO","MT","NE","NV","NJ","NY","ND","OH","OR","SC","SD","TN","TX","UT","VA","WA","WI"],availability:["In Stock","PDF","Online","In-Store"],price:"44% off"},
  {key:"sgc050",name:"Fifth Group Restaurants",brand:"Fifth Group Restaurants",category:"Food And Restaurants",states:["GA"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/1f8f9b92-b70d-4d76-a761-aee6607a0ec3.webp",price:"55% off"},
  {key:"sgc051",name:"Firebirds Wood Fire Grill",brand:"Firebirds Wood Fire Grill",category:"Food And Restaurants",states:["AL","AZ","DE","FL","GA","IN","IA","KS","MD","MO","NE","NJ","NC","OH","OK","PA","SC","TN","VA"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/a1b62ab7-85f1-4695-b280-342afdb628f4.webp",price:"45% off"},
  {key:"sgc052",name:"Firehouse Subs",brand:"Firehouse Subs",category:"Food And Restaurants",states:["AL","AK","AZ","AR","CA","CO","CT","FL","GA","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","PR","RI","SC","SD","TN","TX","UT","VA","WA","WV","WI"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/29d6f0cd-e7cd-4c88-a1a6-34306ed990e3.webp",price:"55% off"},
  {key:"sgc054",name:"Fogo De Chao",brand:"Fogo De Chao",category:"Food And Restaurants",states:["AZ","CA","CO","DC","FL","GA","IL","IN","LA","MD","MA","MI","MN","MO","NV","NY","OR","PA","TX","VA","WA"],availability:["In Stock","Best Selling","PDF","In-Store"],image:"/gc-img/4821992c-9e43-4b23-8e30-cf9d5fde1d66.webp",price:"30% off"},
  {key:"sgc190",name:"Ford's Garage",brand:"Ford's Garage",category:"Food And Restaurants",states:[],availability:["In Stock","PDF","In-Store"],image:"/gc-img/1c6f4093-67bc-49a8-8074-3a1444032dad.webp",price:"30% off"},
  {key:"sgc168",name:"Gordon Ramsay North America (Ramsay Burger, Ramsay Fish & Chips, Hell's Kitchen, Ramsay's Kitchen, Ramsay Steak, Ramsay Street Burger, Ramsay Street Pizza)",brand:"Gordon Ramsay North America (Ramsay Burger, Ramsay Fish & Chips, Hell's Kitchen, Ramsay's Kitchen, Ramsay Steak, Ramsay Street Burger, Ramsay Street Pizza)",category:"Food And Restaurants",states:["CA","CT","DC","FL","IL","MA","MO","NV","NJ","OK","VA"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/434af11e-11ae-4850-ab0d-611ce1457754.webp",price:"30% off"},
  {key:"sgc055",name:"Grimaldi's Coal Brick-oven Pizzeria",brand:"Grimaldi's Coal Brick-oven Pizzeria",category:"Food And Restaurants",states:["AR","CA","FL","ID","KS","KY","NV","NY","SC","TX","WI"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/11192535-7958-4962-b58d-1750e0c4f9e4.webp",price:"49% off"},
  {key:"sgc056",name:"Gyu-kaku Japanese Bbq",brand:"Gyu-kaku Japanese Bbq",category:"Food And Restaurants",states:["AL","AK","AZ","AR","CA","CO","CT","DE","DC","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","PR","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/69c5965a-dc66-4845-b879-6b51190448f0.webp",price:"35% off"},
  {key:"sgc171",name:"Habit Burger & Grill",brand:"Habit Burger & Grill",category:"Food And Restaurants",states:["AZ","CA","FL","ID","MD","NV","NJ","NC","UT","VA","WA"],availability:["In Stock","In-Store"],price:"41% off"},
  {key:"sgc057",name:"Hopdoddy Burger Bar",brand:"Hopdoddy Burger Bar",category:"Food And Restaurants",states:["AZ","CA","CO","FL","LA","TN"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/6148ee4a-1b21-4663-91c3-b4671883835f.webp",price:"45% off"},
  {key:"sgc160",name:"Hungry Howies Pizza",brand:"Hungry Howies Pizza",category:"Food And Restaurants",states:["AL","AZ","AR","CA","CO","DE","FL","GA","IN","LA","MI","MS","NV","NC","OH","OK","PA","SC","TN","TX","UT"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/5c663434-b3d8-43d8-8b6e-b3036a48cc43.webp",price:"45% off"},
  {key:"sgc145",name:"Ihop [SALE]",brand:"Ihop [SALE]",category:"Food And Restaurants",states:["AL","AK","AZ","AR","CA","CO","CT","DE","DC","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","PR","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"],availability:["In Stock","Best Selling","PDF","In-Store"],image:"/gc-img/53aa4b7a-abb2-4d31-a57f-0eb1517a50db.webp",price:"55% off"},
  {key:"sgc174",name:"J Alexander's & Stoney River",brand:"J Alexander's & Stoney River",category:"Food And Restaurants",states:["AL","AZ","CO","FL","GA","IL","KS","KY","LA","MD","MI","MO","NJ","NC","OH","TN","TX"],availability:["In Stock","PDF","In-Store"],price:"34% off"},
  {key:"sgc060",name:"Jason's Deli",brand:"Jason's Deli",category:"Food And Restaurants",states:["AL","AZ","AR","CO","FL","GA","IL","IN","IA","KS","KY","LA","MD","MS","MO","NE","NM","NC","OH","OK","PA","SC","TN","TX","VA","WI"],availability:["In Stock","Best Selling","PDF","In-Store"],image:"/gc-img/f86a6d54-3c00-4452-8ecb-917974256122.webp",price:"45% off"},
  {key:"sgc061",name:"Jim 'n Nick's BBQ",brand:"Jim 'n Nick's BBQ",category:"Food And Restaurants",states:["AL","CO","FL","GA","NC","SC","TN"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/f503c3b8-0e25-4c8e-bb12-14e8fcdfdd71.webp",price:"50% off"},
  {key:"sgc062",name:"Kilwins",brand:"Kilwins",category:"Food And Restaurants",states:["AL","AR","CO","DC","FL","GA","IL","IN","MD","MA","MI","MS","NH","NJ","NY","NC","OH","PA","RI","SC","TN","TX","VA","WI"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/61c896bb-af7f-40d4-b3a0-a71085367c39.webp",price:"50% off"},
  {key:"sgc063",name:"Kona Grill",brand:"Kona Grill",category:"Food And Restaurants",states:["AL","AZ","CO","FL","GA","ID","IL","IN","MD","MI","MN","MO","NE","NV","NJ","OH","TX"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/1ec61cbe-fb97-45db-9053-fe86fb7c6d61.webp",price:"45% off"},
  {key:"sgc064",name:"Krispy Kreme Doughnuts Rewards Cards",brand:"Krispy Kreme Doughnuts Rewards Cards",category:"Food And Restaurants",states:["AL","AK","AZ","AR","CA","CO","CT","DE","DC","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","PR","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/b6b2293a-2c96-4915-8152-e6aa728aeeab.webp",price:"55% off"},
  {key:"sgc065",name:"Kung Fu Tea",brand:"Kung Fu Tea",category:"Food And Restaurants",states:["CA","CO","FL","GA","IN","MD","MI","NJ","OR","PA","TX","WA"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/06b7c996-5310-4c3c-bb9a-d63709b82d93.webp",price:"44% off"},
  {key:"sgc165",name:"Landry's Group (Del Frisco) 600x GCS",brand:"Landry's Group (Del Frisco) 600x GCS",category:"Food And Restaurants",states:["CA","DC","FL","GA","MA","NV","NY","NC","PA","TX"],availability:["In Stock","Best Selling","PDF","Pass2U","In-Store"],image:"/gc-img/c3b08a60-37af-4ba7-9e48-60ed8d03a6db.webp",price:"36% off"},
  {key:"sgc164",name:"Landry's Group (The palm) 0184xx GCS",brand:"Landry's Group (The palm) 0184xx GCS",category:"Food And Restaurants",states:["AL","AZ","AR","CA","CO","DE","DC","FL","GA","HI","ID","IL","IN","KY","LA","MD","MA","MI","MN","MS","MO","NV","NJ","NY","NC","OH","OK","OR","PA","PR","RI","SC","TN","TX","VA","WI"],availability:["In Stock","Best Selling","PDF","In-Store"],image:"/gc-img/27abe7ae-2dd7-4c5a-96c4-b25776318c65.webp",price:"40% off"},
  {key:"sgc067",name:"Lazy Dog",brand:"Lazy Dog",category:"Food And Restaurants",states:["CA","CO","GA","IL","NV","TX","VA"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/b13eb23d-84c0-4bec-a942-480059f68608.webp",price:"45% off"},
  {key:"sgc068",name:"Lucille's Smokehouse Bbq",brand:"Lucille's Smokehouse Bbq",category:"Food And Restaurants",states:["AZ","CA","NV"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/e1b2977e-c683-4d32-9ad3-04de3fa5a6c6.webp",price:"45% off"},
  {key:"sgc069",name:"Lupe Tortilla",brand:"Lupe Tortilla",category:"Food And Restaurants",states:["TX"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/44f679f9-72f1-4e4f-b4da-0f58de256a97.webp",price:"60% off"},
  {key:"sgc070",name:"Mellow Mushroom Pizza",brand:"Mellow Mushroom Pizza",category:"Food And Restaurants",states:["AL","AK","AZ","AR","CA","CO","CT","DE","DC","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","PR","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"],availability:["In Stock","PIN","PDF","In-Store"],image:"/gc-img/4ae6e26f-b9f1-4867-a19d-ae84516cacb1.webp",price:"45% off"},
  {key:"sgc072",name:"Nando's Peri Peri",brand:"Nando's Peri Peri",category:"Food And Restaurants",states:["DC","IL","MD","VA"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/824959c4-df4e-4258-8b6c-010b6e4f137f.webp",price:"50% off"},
  {key:"sgc074",name:"Noodles & Company",brand:"Noodles & Company",category:"Food And Restaurants",states:["AZ","CA","CO","CT","FL","ID","IL","IN","IA","KS","KY","MD","MI","MN","MO","NE","NY","NC","ND","OH","OR","PA","SD","TN","UT","VA","WA","WI"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/8bcc1b04-bb68-4c00-825d-e34002eed4ec.webp",price:"50% off"},
  {key:"sgc163",name:"Ocean Prime",brand:"Ocean Prime",category:"Food And Restaurants",states:["AZ","CA","CO","DC","FL","IL","IN","MA","MI","MO","OH","PA"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/df4966f6-b680-46dc-baef-71887f45f1ab.webp",price:"38% off"},
  {key:"sgc161",name:"Panda Express",brand:"Panda Express",category:"Food And Restaurants",states:["AL","AK","AZ","AR","CA","CO","CT","DE","DC","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","PR","RI","SC","SD","TN","TX","UT","VA","WA","WV","WI","WY"],availability:["In Stock","Pass2U","In-Store"],image:"/gc-img/c47f94e2-ff68-477d-8042-b37d18b4ca70.webp",price:"40% off"},
  {key:"sgc075",name:"Panera Bread",brand:"Panera Bread",category:"Food And Restaurants",states:["AL","AZ","AR","CA","CO","CT","DE","FL","GA","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","RI","SC","SD","TN","TX","VT","VA","WA","WV","WI","WY"],availability:["In Stock","Best Selling","PDF","In-Store"],image:"/gc-img/cf904835-9c6a-4bab-909c-9f40e2a2935b.webp",price:"40% off"},
  {key:"sgc076",name:"Pdq Restaurant",brand:"Pdq Restaurant",category:"Food And Restaurants",states:["FL","NJ","NY","NC","SC"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/cbd4fda1-0b7f-486b-9d03-ff33c0fcc289.webp",price:"45% off"},
  {key:"sgc077",name:"Peet's Coffee & Tea",brand:"Peet's Coffee & Tea",category:"Food And Restaurants",states:["AL","AK","AZ","AR","CA","CO","CT","DE","DC","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","PR","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/86703028-27a5-45c0-8104-43e45d90f1a8.webp",price:"50% off"},
  {key:"sgc078",name:"Pei Wei (USE ONLINE)",brand:"Pei Wei (USE ONLINE)",category:"Food And Restaurants",states:["AZ","AR","CA","CO","FL","IL","IN","KS","MD","MI","MN","NV","NJ","NC","OH","OK","RI","TN","TX","UT","VA"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/fb489207-f958-4cb0-a090-73cb74a48033.webp",price:"55% off"},
  {key:"sgc079",name:"Penn Station Subs",brand:"Penn Station Subs",category:"Food And Restaurants",states:["GA","IL","IN","KS","KY","MI","MO","NC","OH","PA","SC","TN","TX","VA","WV"],availability:["In Stock","PIN","PDF","Online","In-Store"],image:"/gc-img/c8ee9878-0d2b-4680-b37f-227d30887a8d.webp",price:"55% off"},
  {key:"sgc175",name:"Perry's Steakhouse & Grille",brand:"Perry's Steakhouse & Grille",category:"Food And Restaurants",states:["AL","TX","CO","FL","IL","NC","TN"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/a232cd63-8578-4b62-aa30-dacb08c0a30a.webp",price:"41% off"},
  {key:"sgc080",name:"Pf Chang's China Bistro",brand:"Pf Chang's China Bistro",category:"Food And Restaurants",states:["AL","AK","AZ","AR","CA","CO","CT","DE","DC","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","PR","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"],availability:["In Stock","Best Selling","PDF","In-Store"],image:"/gc-img/78c8dbe5-5fba-415c-8d88-a5bfb909ea3f.webp",price:"55% off"},
  {key:"sgc081",name:"Piada Italian Street Food",brand:"Piada Italian Street Food",category:"Food And Restaurants",states:["AL","AK","AZ","AR","CA","CO","CT","DE","DC","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","PR","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/aa9500a7-b3fd-40ed-9ae3-809ea236e197.webp",price:"45% off"},
  {key:"sgc082",name:"Portillo's",brand:"Portillo's",category:"Food And Restaurants",states:["AZ","CA","FL","IL","IN","WI"],availability:["In Stock","Best Selling","PDF","In-Store"],image:"/gc-img/cf04c53f-36b9-4575-bea1-fd7bb8315a57.webp",price:"45% off"},
  {key:"sgc083",name:"Potbelly Sandwich",brand:"Potbelly Sandwich",category:"Food And Restaurants",states:["AZ","AR","CO","CT","DC","IL","IN","IA","KS","KY","MD","MA","MI","MN","OR","PA","SD","TX","UT","VA","WA","WI"],availability:["In Stock","Best Selling","PDF","In-Store"],image:"/gc-img/243fc9d6-4333-4c3a-81b8-9ebceff886e9.webp",price:"60% off"},
  {key:"sgc084",name:"Quality Branded Restaurants (quality Italian, Quality Meats, Quality Bistro, Don Angie, Zou Zou's, Smith & Wollensky, Kini's, Cretans, Chez Roc, Bad Romam)",brand:"Quality Branded Restaurants (quality Italian, Quality Meats, Quality Bistro, Don Angie, Zou Zou's, Smith & Wollensky, Kini's, Cretans, Chez Roc, Bad Romam)",category:"Food And Restaurants",states:["CA","FL","NY"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/a1a15cb9-7209-4805-9a41-9b8bdb895c64.webp",price:"45% off"},
  {key:"sgc143",name:"Raising Canes",brand:"Raising Canes",category:"Food And Restaurants",states:["AL","AZ","CA","CO","GA","IA","LA","MN","MS","MT","NV","NM","OH","OK","OR","SC","TX","UT","VA"],availability:["In Stock","Best Selling","In-Store"],image:"/gc-img/b99ea6c7-5d34-47a7-a97b-dcd67124f6e2.webp",price:"45% off"},
  {key:"sgc085",name:"Rock & Brews",brand:"Rock & Brews",category:"Food And Restaurants",states:["CA","FL","KS","MO","TX"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/f0cad124-00ce-404d-8234-ba1cb00a333a.webp",price:"45% off"},
  {key:"sgc086",name:"Romano's Macaroni Grill",brand:"Romano's Macaroni Grill",category:"Food And Restaurants",states:["AZ","CA","CO","FL","GA","HI","IL","ME","NV","OH","PA","TN","TX","UT","VA"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/dceae8ef-8d0c-4cbb-b32b-224083bb705e.webp",price:"60% off"},
  {key:"sgc087",name:"Ruby Tuesday",brand:"Ruby Tuesday",category:"Food And Restaurants",states:["FL","GA","IN","MD","MO","NC","OH","PA","SC","TN","VA"],availability:["In Stock","PDF","Online","In-Store"],image:"/gc-img/d4414444-8a97-4675-8a21-a77b946edbd6.webp",price:"55% off"},
  {key:"sgc088",name:"Ruth's Chris",brand:"Ruth's Chris",category:"Food And Restaurants",states:["AL","AK","AZ","AR","CA","CO","CT","DE","DC","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","PR","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"],availability:["In Stock","Best Selling","PDF","In-Store"],image:"/gc-img/54eac716-b8bb-47f7-9546-74cd04ac63e6.webp",price:"40% off"},
  {key:"sgc089",name:"Salad And Go",brand:"Salad And Go",category:"Food And Restaurants",states:["AZ","TX"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/04ce2294-ffb9-46b1-8774-b606c72fb5fe.webp",price:"55% off"},
  {key:"sgc091",name:"Sizzler",brand:"Sizzler",category:"Food And Restaurants",states:["AZ","CA","ID","NV","NM","OR","UT","WA"],availability:["In Stock","Pass2U","In-Store"],image:"/gc-img/ba7b4040-61c6-4497-afe6-a2d7e81ed2d1.webp",price:"60% off"},
  {key:"sgc176",name:"Smashburger",brand:"Smashburger",category:"Food And Restaurants",states:["AL","AZ","AR","CA","CO","CT","DC","FL","ID","IL","IN","IA","KS","KY","LA","MD","MA","MI","MN","MO","NV","NJ","NY","NC","ND","OH","OK","PA","RI","SC","TX","UT","VA","WA"],availability:["In Stock","Best Selling","PIN","PDF","Online","In-Store"],image:"/gc-img/a75466e6-4305-46ad-a8f9-39989ae45226.webp",price:"60% off"},
  {key:"sgc093",name:"Smith & Wollensky [Excludes NYC]",brand:"Smith & Wollensky [Excludes NYC]",category:"Food And Restaurants",states:["FL","IL","MA","NV","OH"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/e4443589-f77b-4ac3-a665-d9824c894c8c.webp",price:"45% off"},
  {key:"sgc094",name:"Snooze Am Eatery",brand:"Snooze Am Eatery",category:"Food And Restaurants",states:["AZ","CA","CO","GA","MO","NC","TX"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/fce20082-22d9-4af4-b212-58b04fd758a9.webp",price:"55% off"},
  {key:"sgc095",name:"Spb Hospitality Group (a1a Ale Works, Big River Grill, Chophouse & Brewery, Gordon Biersch, Logan's Roadhouse, Stoney River, Ragtime Tavern, Redlands Grill, Rock Bottom, Merus Grill, Old Chicago Pizza)",brand:"Spb Hospitality Group (a1a Ale Works, Big River Grill, Chophouse & Brewery, Gordon Biersch, Logan's Roadhouse, Stoney River, Ragtime Tavern, Redlands Grill, Rock Bottom, Merus Grill, Old Chicago Pizza)",category:"Food And Restaurants",states:["AL","AR","CA","CO","FL","GA","ID","IL","IN","IA","KS","KY","LA","MD","MA","MI","MN","MS","MO","MT","NE","NV","NC","OH","OK","OR","PA","SC","SD","TN","TX","VA","WV","WI","WY"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/4ec51b40-e9f6-4750-8107-956052bd5b0c.webp",price:"45% off"},
  {key:"sgc097",name:"Starr Restaurants",brand:"Starr Restaurants",category:"Food And Restaurants",states:["PA"],availability:["In Stock","PIN","PDF","Online","In-Store"],image:"/gc-img/0fab58c1-32e2-4796-865b-083cfe29869a.webp",price:"44% off"},
  {key:"sgc098",name:"Stk Steakhouse",brand:"Stk Steakhouse",category:"Food And Restaurants",states:["AZ","CA","CO","FL","GA","IL","NV","NY","PR","TN","TX","WA"],availability:["In Stock","Best Selling","PDF","In-Store"],image:"/gc-img/e1aab810-b6b3-462a-9fd8-a9a9715d82f8.webp",price:"30% off"},
  {key:"sgc147",name:"Sullivan's Steakhouse",brand:"Sullivan's Steakhouse",category:"Food And Restaurants",states:["AK","AZ","AR","CA","DE","IL","KS","LA","NE","NC","PA"],availability:["In Stock"],price:"40% off"},
  {key:"sgc169",name:"Sweetgreen",brand:"Sweetgreen",category:"Food And Restaurants",states:["CA","CO","CT","DC","FL","GA","IL","MD","MA","NJ","NY","PA","TX","VA"],availability:["In Stock","In-Store"],image:"/gc-img/2c43a120-7c24-4f5b-8bb5-096193b85917.webp",price:"36% off"},
  {key:"sgc099",name:"T S Restaurants",brand:"T S Restaurants",category:"Food And Restaurants",states:["CA","HI"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/618c5a6e-ce70-45aa-a023-34856b5cec7b.webp",price:"45% off"},
  {key:"sgc101",name:"Tender Greens",brand:"Tender Greens",category:"Food And Restaurants",states:["CA","MA"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/38b89a2d-d7e0-4739-bfe0-50e286c0d514.webp",price:"50% off"},
  {key:"sgc102",name:"Texas Roadhouse & Bubba's 33",brand:"Texas Roadhouse & Bubba's 33",category:"Food And Restaurants",states:["AL","AK","AZ","AR","CA","CO","CT","DE","FL","GA","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NM","NY","NC","ND","OH","OK","OR","PA","RI","SC","SD","TN","TX","UT","VT","VA","WV","WI","WY"],availability:["In Stock","Best Selling","PIN","PDF","Online","In-Store"],image:"/gc-img/d253cb23-475b-4d0f-b578-5b9a416c7a01.webp",price:"22% off"},
  {key:"sgc104",name:"The Honey Baked Ham",brand:"The Honey Baked Ham",category:"Food And Restaurants",states:["AL","AK","AZ","AR","CA","CO","CT","DE","DC","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","PR","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/9c303e9a-8b4f-445f-97c2-9058c080bb7b.webp",price:"50% off"},
  {key:"sgc107",name:"Toojay's Deli",brand:"Toojay's Deli",category:"Food And Restaurants",states:["FL"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/6c42e41a-d22b-4484-8eda-7238f6ebb6d7.webp",price:"55% off"},
  {key:"sgc108",name:"Torchy's Tacos",brand:"Torchy's Tacos",category:"Food And Restaurants",states:["AR","CO","KS","LA","MO","OK","TX"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/df0d2fb1-9f3f-4e21-9004-86b2589e4318.webp",price:"50% off"},
  {key:"sgc109",name:"Tropical Smoothie Cafe",brand:"Tropical Smoothie Cafe",category:"Food And Restaurants",states:["AL","AZ","AR","CA","CO","CT","DE","DC","FL","GA","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO"],availability:["In Stock","Best Selling","PIN","PDF","In-Store"],image:"/gc-img/44a74d2c-8770-4ba8-9b07-300adb4c0f1e.webp",price:"35% off"},
  {key:"sgc110",name:"True Food Kitchen",brand:"True Food Kitchen",category:"Food And Restaurants",states:["AZ","CA","CO","FL","GA","IL","LA","MD","MO","NV","NJ","NY","OH","PA","TN","TX","VA"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/0cbdb7b4-6ad1-4961-aa5f-a7aec07ac861.webp",price:"60% off"},
  {key:"sgc111",name:"Twin Peaks",brand:"Twin Peaks",category:"Food And Restaurants",states:["AL","AZ","AR","CA","CO","FL","GA","ID","IL","IN","IA","KS","KY","LA","MI","MS","MO","NV","NM","NC","ND","OH","OK","SC","TN","TX","WA"],availability:["In Stock","Best Selling","PDF","In-Store"],image:"/gc-img/1e227d74-324a-4b6e-95de-f5fafd9c2caa.webp",price:"40% off"},
  {key:"sgc112",name:"Uncle Julio's",brand:"Uncle Julio's",category:"Food And Restaurants",states:["CO","FL","IL","MD","MO","NJ","NC","OK","TN","TX","VA","WI"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/3890671a-ebbb-4c8f-8169-36dc1034fc2e.webp",price:"40% off"},
  {key:"sgc113",name:"Uno Pizzeria & Grill",brand:"Uno Pizzeria & Grill",category:"Food And Restaurants",states:["CO","DC","FL","IL","IN","ME","MD","MA","MI","NH","NJ","NY","OH","PA","RI","SC","VT","VA","WI"],availability:["Out of Stock","PDF","Online","In-Store"],image:"/gc-img/b2bcebeb-ee43-4246-bd6c-dd3fb27bb8fc.webp"},
  {key:"sgc114",name:"Velvet Taco",brand:"Velvet Taco",category:"Food And Restaurants",states:["GA","IL","TX"],availability:["In Stock","PDF","Online","In-Store"],image:"/gc-img/96f3fc64-81e9-4d18-a5b7-6bf496a9e907.webp",price:"45% off"},
  {key:"sgc115",name:"Whataburger",brand:"Whataburger",category:"Food And Restaurants",states:["AL","AZ","AR","FL","GA","LA","MS","NM","OK","TX"],availability:["In Stock","Best Selling","PDF","In-Store"],image:"/gc-img/2be70573-3eb1-452b-8e8d-30a146a9c875.webp",price:"50% off"},
  {key:"sgc116",name:"Xperience Restaurant Group (el Torito, Acapulco Mexican Restaurants, El Torito Grill, Las Brisas, Sinigual, And Who Song & Larry's)",brand:"Xperience Restaurant Group (el Torito, Acapulco Mexican Restaurants, El Torito Grill, Las Brisas, Sinigual, And Who Song & Larry's)",category:"Food And Restaurants",states:["CA","FL","IL","MD","MN","NJ","NY","SD","WA"],availability:["In Stock","PDF","In-Store"],image:"/gc-img/36d1cda4-7421-40db-a84c-89c77fa98e7e.webp",price:"45% off"},
  {key:"sgc118",name:"Zaxby's",brand:"Zaxby's",category:"Food And Restaurants",states:["AL","AR","FL","GA","IN","KS","KY","LA","MS","MO","NC","OK","SC","TN","TX","UT","VA"],availability:["In Stock","Best Selling","PDF","In-Store"],image:"/gc-img/c8e42226-17bf-4ea4-9f98-fd48ac1c9ffe.webp",price:"55% off"},
  {key:"sgc119",name:"Racetrac",brand:"Racetrac",category:"Gas Stations",states:["AL","AR","FL","GA","IN","KY","LA","MS","NC","OH","SC","TN","TX","VA"],availability:["In Stock","Best Selling","PDF","In-Store"],image:"/gc-img/ea9c21dc-518c-4c38-8272-e4f5ac899540.webp",price:"25% off"},
  {key:"sgc180",name:"Sheetz",brand:"Sheetz",category:"Gas Stations",states:["MD","MI","NC","OH","PA","VA","WV"],availability:["In Stock","Best Selling","PDF"],image:"/gc-img/03bf0531-20dc-4949-bb1f-0838d855c442.webp",price:"25% off"},
  {key:"sgc125",name:"American Girl",brand:"American Girl",category:"Retail",states:["CA","CO","DC","GA","IL","LA","MA","MN","MO","NV","NM","NY","TN","TX","WA"],availability:["In Stock","Best Selling","PDF"],image:"/gc-img/7eed5165-9cbb-47f5-891a-3b47b403dbb8.webp",price:"50% off"},
  {key:"sgc126",name:"Ann Taylor / Loft",brand:"Ann Taylor / Loft",category:"Retail",states:["AL","AZ","CA","CO","CT","DC","FL","GA","IL","IN","KY","LA","MD","MA","MI","MN","MO","NE","NV","NH","NJ","NY","NC","OH","OR","PA","RI","TN","TX","UT","VA","WA","WI"],availability:["In Stock","Best Selling","Online"],image:"/gc-img/9c650412-98e5-4196-9ef3-82371bb1e6ba.webp",price:"49% off"},
  {key:"sgc127",name:"Arc'teryx",brand:"Arc'teryx",category:"Retail",states:["CA","CO","DC","IL","MD","MA","MN","NJ","NY","OR","UT","VA","WA"],availability:["In Stock","Best Selling","Pass2U","In-Store"],image:"/gc-img/1441617d-ea9d-47b1-ba35-4106acff89e2.webp",price:"40% off"},
  {key:"sgc139",name:"Aritzia",brand:"Aritzia",category:"Retail",states:[],availability:["In Stock","Best Selling","Pass2U","In-Store"],image:"/gc-img/1dc57df1-4413-4222-b4a4-e506d2a1a2b4.webp",price:"35% off"},
  {key:"sgc166",name:"Burlington Coat Factory",brand:"Burlington Coat Factory",category:"Retail",states:["AL","AK","AZ","AR","CA","CO","CT","DE","DC","FL","GA","ID","IL","IN","IA","KS","KY","LA","MD","MA","MI","MN","MS","MO","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","PR","RI","SC","SD","TN","TX","UT","VA","WA","WV","WI"],availability:["In Stock","Best Selling","PDF"],image:"/gc-img/50ba44b2-d8a4-4db2-9e0b-5f460c527b98.webp",price:"53% off"},
  {key:"sgc129",name:"Carhartt",brand:"Carhartt",category:"Retail",states:["CT","FL","ID","IL","IN","ME","MD","MA","MI","MN","MO","NE","NJ","NY","OH","OR","PA","TN","TX","UT","VA","WA","WI"],availability:["In Stock","Best Selling","PDF"],image:"/gc-img/2dc5979a-b4cb-488e-b7b3-460d3354131c.webp",price:"49% off"},
  {key:"sgc130",name:"Designer Shoe Wholesale (dsw)",brand:"Designer Shoe Wholesale (dsw)",category:"Retail",states:["AL","AZ","AR","CA","CO","CT","DE","DC","FL","GA","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","NE","NH","NM","NC","OH","OR","RI","SD","TX","VA"],availability:["In Stock","Best Selling","PIN","PDF"],image:"/gc-img/feadbedc-96a9-48ed-9b21-eae9dd8c4ff7.webp",price:"34% off"},
  {key:"sgc131",name:"Fleetfeet",brand:"Fleetfeet",category:"Retail",states:[],availability:["In Stock","Best Selling","Pass2U","In-Store"],image:"/gc-img/37b691e6-89fe-4969-907f-0183ba43a005.webp",price:"45% off"},
  {key:"sgc183",name:"Kirkland's",brand:"Kirkland's",category:"Retail",states:["AL","AK","AZ","CA","CO","CT","DE","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"],availability:["In Stock","Best Selling","PDF","In-Store"],image:"/gc-img/3fb56748-17f8-43ce-8850-13f0f5e51265.webp",price:"35% off"},
  {key:"sgc133",name:"Lowes",brand:"Lowes",category:"Retail",states:["AL","AK","AZ","AR","CA","CO","CT","DE","DC","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","PR","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"],availability:["In Stock","Best Selling","PIN","PDF","Online","In-Store"],image:"/gc-img/4ce413a0-d15d-40c2-8e3f-8a03d85db964.webp",price:"5% off"},
  {key:"sgc134",name:"Ocean State Job Lot",brand:"Ocean State Job Lot",category:"Retail",states:[],availability:["In Stock","Pass2U","Online","In-Store"],image:"/gc-img/017c3bf4-1cd9-4b83-856d-8a076cbac238.webp",price:"55% off"},
  {key:"sgc136",name:"Pandora",brand:"Pandora",category:"Retail",states:["AL","AK","AZ","AR","CA","CO","CT","DE","DC","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"],availability:["In Stock","Best Selling","Pass2U","In-Store"],image:"/gc-img/82443a8d-6f98-4a0a-92cc-a92a87eae168.webp",price:"34% off"},
  {key:"sgc137",name:"Staples",brand:"Staples",category:"Retail",states:["AL","AZ","AR","CA","CO","CT","DE","FL","GA","ID","IL","IN","IA","KS","KY","ME","MD","MA","MI","MO","MT","NE","NV","NH","NJ","NM","NY","NC","OH","OK","OR","PA","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"],availability:["In Stock","Best Selling","Pass2U","In-Store"],image:"/gc-img/174a6a1e-da8b-46aa-92ac-76ad3d64cc33.webp",price:"15% off"},
  {key:"sgc138",name:"Victoria's Secret",brand:"Victoria's Secret",category:"Retail",states:["AL","AK","AZ","AR","CA","CO","CT","DE","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","PR","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"],availability:["In Stock","Best Selling","PDF","In-Store"],image:"/gc-img/7de6fd49-b558-4523-a930-24d8d5e99a9b.webp",price:"35% off"},
  {key:"sgc124",name:"World Market",brand:"World Market",category:"Groceries",states:["AL","AZ","CA","CO","CT","FL","GA","ID"],availability:["In Stock","Best Selling","PDF","In-Store"],image:"/gc-img/cd2d6160-e198-4129-ac2a-c272afefccdd.webp",price:"45% off"},
  {key:"sgc151",name:"AMC Movie Theaters",brand:"AMC Movie Theaters",category:"Entertainment",states:[],availability:["Out of Stock","PIN","Online","In-Store"],image:"/gc-img/03827f14-7af7-41a1-b7fc-81bf6a07a202.webp"},
  {key:"sgc002",name:"Cinemark (century Theatres, Cinearts, Tinseltown, Rave Cinemas)",brand:"Cinemark (century Theatres, Cinearts, Tinseltown, Rave Cinemas)",category:"Entertainment",states:["AL","AK","AZ","CA","CO","DE","FL","GA","IL","IN","IA","KS","KY","LA","MD","MA","MI","MN","MS","MT","NV","NH","NJ","NM","NY","NC","OH","OK","OR","PA","SC","SD","TN","TX","UT","VA","WA","WV","WI"],availability:["In Stock","Best Selling","PDF","Online","In-Store"],image:"/gc-img/368ffaf6-6455-43dc-8017-9b95c63def5c.webp",price:"45% off"},
  {key:"sgc003",name:"Craft Warehouse (Online)",brand:"Craft Warehouse (Online)",category:"Entertainment",states:[],availability:["In Stock","PIN","Online"],image:"/gc-img/d2259ffc-0001-4cdb-baae-0136d3cd6b4f.webp",price:"55% off"},
  {key:"sgc167",name:"Dave & Busters",brand:"Dave & Busters",category:"Entertainment",states:["AL","AK","AR","CA","CO","CT","FL","GA","HI","ID","IL","IN","KS","KY","LA","MD","MA","MI","MN","MO","NE","NV","NH","NJ","NM","NY","NC","OH","OK","OR","PA","PR","RI","SC","SD","TN","TX","UT","VA","WA","WI"],availability:["In Stock","Best Selling","PDF","In-Store"],price:"40% off"},
  {key:"sgc004",name:"Disney (disneyland, Disney Store, Disney Plus)",brand:"Disney (disneyland, Disney Store, Disney Plus)",category:"Entertainment",states:["AL","AK","AZ","AR","CA","CO","CT","DE","DC","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","PR","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"],availability:["In Stock","Best Selling","PIN"],image:"/gc-img/2ad6ad9f-a5aa-4832-9234-c51cb300dbe8.webp",price:"20% off"},
  {key:"sgc005",name:"Emagine Entertainment",brand:"Emagine Entertainment",category:"Entertainment",states:["IL","MI","MN","WI"],availability:["In Stock","Best Selling","PDF"],image:"/gc-img/e238b93e-4597-41d9-abcf-ce395fe89dcd.webp",price:"45% off"},
  {key:"sgc006",name:"Fandango",brand:"Fandango",category:"Entertainment",states:["AL","AK","AZ","AR","CA","CO","CT","DE","DC","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","PR","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"],availability:["In Stock","Best Selling","Online","In-Store"],image:"/gc-img/a54f6cd6-888c-4300-995b-bd0bef17efbb.webp",price:"40% off"},
  {key:"sgc142",name:"Regal Cinemas [SALE]",brand:"Regal Cinemas [SALE]",category:"Entertainment",states:["AL","AK","AZ","CA","CO","CT","DE","DC","FL","GA","HI","ID","IL","IN","KS","KY","LA","ME","MD","MA","MI","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","OH","OK","OR","PA","SC","TN","TX","UT","VA","WA","WV","WY"],availability:["In Stock","Best Selling","PDF","In-Store"],image:"/gc-img/a11a7a14-08c8-45da-bb27-09495c92447e.webp",price:"52% off"},
  {key:"sgc184",name:"Sea World (busch Gardens, Aquatica, Adventure Island, Discovery Cove, Sesame Palace, Water Country)",brand:"Sea World (busch Gardens, Aquatica, Adventure Island, Discovery Cove, Sesame Palace, Water Country)",category:"Entertainment",states:["CA","FL","PA","TX","VA"],availability:["In Stock","Best Selling","In-Store"],image:"/gc-img/ff4b831d-8313-425b-b8cc-531733fefe53.webp",price:"35% off"},
  {key:"sgc009",name:"Studio Movie Grill",brand:"Studio Movie Grill",category:"Entertainment",states:["CA","FL","GA","IL","PA","TX"],availability:["In Stock","Best Selling","PDF","Online","In-Store"],image:"/gc-img/76bf7045-30d9-45ea-80e4-bda5479e8c23.webp",price:"50% off"},
  {key:"sgc178",name:"Autozone",brand:"Autozone",category:"Exclusive",states:["AL","AK","AS","AZ","AR","CA","CO","CT","DE","DC","FM","FL","GA","GU","HI","ID","IL","IN","IA","KS","KY","LA","ME","MH","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","MP","OH","OK","OR","PW","PA","PR","RI","SC","SD","TN","TX","UT","VT","VI","VA","WA","WV","WI","WY"],availability:["In Stock","Best Selling","PDF"],price:"30% off"},
  {key:"sgc162",name:"Chicos / Soma / White House Black Market",brand:"Chicos / Soma / White House Black Market",category:"Entertainment And Shopping",states:["AL","AK","AZ","AR","CA","CO","CT","DE","DC","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","PR","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"],availability:["In Stock","Best Selling","PIN","Online","In-Store"],image:"/gc-img/f839313d-d57b-4515-b4d1-6c9c778be256.webp",price:"30% off"},
  {key:"sgc187",name:"Guitar Center",brand:"Guitar Center",category:"Exclusive",states:["AL","AK","AZ","CA","CO","CT","DE","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MI","MN","MS","MO","NE","NJ","NM","NY","NC","ND","OH","OR","PA","RI","SC","SD","TN","TX","UT","VT","VA","WA","WI"],availability:["In Stock","PDF","In-Store"],price:"20% off"},
  {key:"sgc188",name:"Petsmart",brand:"Petsmart",category:"Exclusive",states:["AL","AR","AZ","CA","CO","CT","DC","DE","FL","GA","HI","IA","ID","IL","IN","KS","KY","LA","MA","MD","ME","MI","MN","MO","MS","MT","NC","ND","NE","NH","NJ","NM","NV","NY","OH","OK","OR","PA","RI","SC","SD","TN","TX","UT","VA","VT","WA","WI","WV","WY"],availability:["In Stock","Best Selling","PDF"],price:"20% off"},
];
...
const CARDS_PER_PAGE = 24;
  ...
  function GiftCardsSection() {
    ...
    const [showAdd, setShowAdd] = useState(false);
    const [addName, setAddName] = useState("");
    const [addCategory, setAddCategory] = useState("Food And Restaurants");
    const [addPrice, setAddPrice] = useState("");
    const [addImage, setAddImage] = useState("");

    const [deleted, setDeleted] = useState<string[]>(() => {
      try { return JSON.parse(getValue("buy4u.giftcards.deleted", "[]")); } catch { return []; }
    });
    const [extra, setExtra] = useState<GiftCard[]>(() => {
    ...
    return (
      <section id="buy4u-giftcards" className="scroll-mt-24 rounded-3xl border border-white/10 bg-white/[0.03] p-6 sm:p-8">
        ...
        {allCards.length === 0 && !(isAdmin && editMode) ? (
          ...
          <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
            {shown.map(c => (
              <div key={c.key} className={`group relative rounded-2xl border border-white/10 bg-white/[0.04] p-3${isAdmin && editMode ? " ring-1 ring-transparent hover:ring-amber-300/40" : ""}`}>
                {isAdmin && editMode && (
                ...
                {c.image&&<Image src={c.image} alt={c.name} width={400} height={300} sizes="(max-width: 640px) 50vw, (max-width: 768px) 33vw, (max-width: 1024px) 25vw, 200px" className="aspect-[4/3] w-full rounded-xl object-cover" unoptimized /* Sources are already ≤256px logo thumbs; re-encoding the whole grid after every restart was the Render 512MB OOM herd (webp-only helped, didn't cure). */ />}
            ...
            {isAdmin && editMode && !showAdd && (
              <button onClick={() => setShowAdd(true)} className="flex min-h-[100px] flex-col items-center justify-center gap-1 rounded-2xl border-2 border-dashed border-white/20 bg-transparent text-white/40 transition hover:border-amber-300/50 hover:text-amber-300/70">
                <span className="text-2xl leading-none">+</span>
                <span className="text-[11px]">Add Card</span>
              </button>
            )}
          ...
          {isAdmin && editMode && showAdd && (
            <div className="mt-3 space-y-3 rounded-2xl border border-amber-300/30 bg-white/[0.05] p-4">
              ...
              <select value={addCategory} onChange={e => setAddCategory(e.target.value)}
                className="w-full rounded-lg border border-white/10 bg-white/10 px-3 py-2 text-sm text-white outline-none focus:border-amber-300/50">
                {GC_CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
              <input value={addPrice} onChange={e => setAddPrice(e.target.value)} placeholder="Discount (e.g. 40% off)"
                className="w-full rounded-lg border border-white/10 bg-white/10 px-3 py-2 text-sm text-white placeholder:text-white/30 outline-none focus:border-amber-300/50" />
              <input value={addImage} onChange={e => setAddImage(e.target.value)} placeholder="Image URL (optional)"
                className="w-full rounded-lg border border-white/10 bg-white/10 px-3 py-2 text-sm text-white placeholder:text-white/30 outline-none focus:border-amber-300/50" />
              <div className="flex gap-2">
                <button onClick={handleAdd} className="rounded-lg bg-amber-400/90 px-4 py-2 text-xs font-bold text-black hover:bg-amber-300 transition-colors">Add Card</button>
                <button onClick={() => setShowAdd(false)} className="rounded-lg border border-white/20 px-4 py-2 text-xs text-white/60 hover:text-white transition-colors">Cancel</button>
