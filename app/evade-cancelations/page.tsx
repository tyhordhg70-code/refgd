...
export default function EvadePage() {
  return (
    <ReorderableContainer pageId="evade-cancelations">
      ...
      {/* Page-wide deep blue → purple atmospheric tint. Used to be a
          near-opaque dark linear gradient (0.65 → 0.88 alpha) which
          dimmed the cosmic backdrop AND every box card on the page,
          making the user report "box cards and text still missing".
          Reduced the linear backdrop to ~10-22% alpha so the cosmic
          galaxy still shines through; kept the violet/purple radial
          accents for the "vault" atmosphere. */}
      <div
        aria-hidden
        className="pointer-events-none fixed inset-0 -z-[1]"
        style={{
          background:
            "radial-gradient(ellipse 80% 60% at 20% 0%, rgba(30,18,90,0.45), transparent 60%), radial-gradient(ellipse 70% 60% at 80% 100%, rgba(82,28,140,0.45), transparent 60%), linear-gradient(180deg, rgba(8,8,32,0.10) 0%, rgba(18,8,40,0.18) 50%, rgba(10,4,32,0.22) 100%)",
        }}
      />
      ...
      <ReorderableSection sectionId="hero">
        ...
        <ChipScroll
          // The /sequence/evade frame folder is intentionally not in
          // the deploy bundle (would be ~5 MB of webp). Setting
          // frameCount={0} short-circuits the loader inside ChipScroll
          // straight to the cinematic procedural "shield" scene — no
          // 6-second spinner, no 15+ console 404s, instant first paint.
          dir="/sequence/evade"
          frameCount={0}
          background="#05060a"
          accent="#22d3ee"
