        ...
        <ReorderableSection sectionId="telegram">
          ...
          >
            ...
            */}
            ...
            {/* Decorative fade-out gradient at the very bottom of the
                home page. The home page has NO footer (Footer.tsx
                returns null on `/`) so this gradient previously ended
                at `rgb(10,12,20)` (#0a0c14, ink-900) and then the body
                bg-ink-950 (#05060a) took over below. The 4-point
                lightness difference between #0a0c14 and #05060a
                rendered as a visible HORIZONTAL DARK STRIP across the
                bottom of the page — the user's "black strip when
                scrolling down on home" report.
                Fix: end the gradient on rgba(5,6,10,0) — fully
                transparent — so it dissolves into the body bg with
                zero color step. Now invisible against the cosmic
                background but still safe-to-leave-in-place if a footer
                is ever added back. */}
            <div
              aria-hidden="true"
              ...
              style={{
                background:
                  "linear-gradient(to bottom, rgba(5,6,10,0) 0%, rgba(5,6,10,0.35) 45%, rgba(5,6,10,0) 100%)",
              }}
            />
          </ParallaxChapter>
        </ReorderableSection>
