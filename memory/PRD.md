...
- 2026-05-02: Reverted incorrect refgd/ port (37754de) so live site actually shows 09b9c48 rollback (commit 92f38d1).
- 2026-05-02: Removed duplicate root-level `/app` and `/components` directories (commit 92f38d1).
- 2026-05-02: Removed leftover `refgd_deploy_fix.patch` from previous session (commit 92f38d1).
- 2026-05-02: **Fixed planet visibility** — `GalaxyBackground.tsx` had the WebGL canvas rendered FIRST in DOM with dark vignette gradients painted ON TOP of it. The worker's planet/halo/nebulas/warp streaks were always being rendered but completely hidden by the dark overlays. Reordered children (canvas last) and added `isolation: isolate` to the wrapper so `mix-blend-mode: screen` blends with the gradient siblings instead of the page backdrop. Commits 44554ca + 57304f0. Verified live: planet now fully visible behind the WELCOME headline.
